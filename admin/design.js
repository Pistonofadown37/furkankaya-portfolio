...
(function () {
  ...
  const DEFAULTS = {
    ...
    mobile_hero_gap:0,mobile_header_height:82,mobile_side_padding:20,mobile_logo_font_size:17,mobile_logo_gap:4,mobile_hero_title_size:52,mobile_hero_title_size_small:52,mobile_hero_description_size:14,mobile_hero_description_size_small:14,mobile_hero_visual_height:330,mobile_hero_padding_top:100,mobile_hero_padding_bottom:20,mobile_brand_size:180,mobile_brand_spacing:-18,mobile_brand_name_size:35,mobile_brand_name_spacing:9,mobile_section_padding:80,mobile_section_title_size:34,mobile_section_title_size_small:30,mobile_portfolio_columns:1,mobile_portfolio_gap:16,mobile_portfolio_image_height:300,mobile_portfolio_image_height_small:240,mobile_service_columns:1,mobile_service_gap:16,mobile_service_card_padding:22,mobile_service_card_gap:16,mobile_service_card_min_height:0,mobile_service_icon_size:50,mobile_contact_padding:25,mobile_contact_gap:24,mobile_contact_link_height:82,mobile_button_height:58,mobile_button_gap:16
  };
  ...
  async function saveOneSetting(fullKey, value){
    const settingValue = String(value ?? "");

    // Mevcut tüm kayıtları güncelle. Eski sürümlerde aynı key'den
    // birden fazla kayıt oluşmuşsa hepsinin aynı değere gelmesini sağlar.
    ...
    if (!check || !check.length) {
      throw new Error(fullKey + " veritabanında bulunamadı.");
    }

    const expected = String(settingValue);
    const mismatch = check.find(row => String(row.setting_value ?? "") !== expected);
    if (mismatch) {
      throw new Error(fullKey + " veritabanında doğrulanamadı.");
    }
  }
  ...
  async function save(){
    ...
    try{
      // Marka alanlarını doğrudan formdan tekrar oku.
      // Böylece input -> settings senkronizasyonundan bağımsız olarak
      // kullanıcının ekranda gördüğü son değer kesin olarak kaydedilir.
      const brandInputs = {
        brand_letter_1: document.querySelector('[data-key="brand_letter_1"]'),
        brand_letter_2: document.querySelector('[data-key="brand_letter_2"]'),
        brand_name: document.querySelector('[data-key="brand_name"]')
      };

      for (const [key, input] of Object.entries(brandInputs)) {
        if (!input) throw new Error(key + " alanı bulunamadı.");
        settings[key] = input.value;
        await saveOneSetting("design_" + key, input.value);
      }

      // Marka dışındaki tüm tasarım/içerik ayarlarını mevcut sistemle kaydet.
      const rows=Object.entries(settings)
        .filter(([key]) => !["brand_letter_1","brand_letter_2","brand_name"].includes(key))
        .map(([key,value])=>({
      ...
      (existing||[]).forEach(row=>{
        if(!existingMap.has(row.setting_key))existingMap.set(row.setting_key,[]);
        existingMap.get(row.setting_key).push(row.id);
      });

      const inserts=[];
      const updates=[];
      for(const row of rows){
        const ids=existingMap.get(row.setting_key)||[];
        if(ids.length){
          ids.forEach(id=>updates.push({id,value:row.setting_value}));
        }else{
          inserts.push(row);
        }
      }
      ...
      for(let i=0;i<updates.length;i+=batchSize){
        ...
        const results=await Promise.all(batch.map(row=>
          client.from("site_settings")
            .update({setting_value:row.value})
            .eq("id",row.id)
        ));
  ...
  function tabs(){
    ...
    function activate(name){
      const target=[...ts].find(t=>t.dataset.panel===name) || ts[0];
      if(!target)return;
      ts.forEach(x=>x.classList.remove("active"));
      ps.forEach(x=>x.classList.remove("active"));
      target.classList.add("active");
      document.querySelector('[data-panel-content="'+target.dataset.panel+'"]')?.classList.add("active");
    }
    ts.forEach(t=>t.addEventListener("click",()=>activate(t.dataset.panel)));
    const requested=new URLSearchParams(location.search).get("tab");
    activate(requested || ts[0]?.dataset.panel);
  }
