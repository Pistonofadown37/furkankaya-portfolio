...
(function () {
  ...
  async function save(){
    ...
    try{
      /*
       * Metin marka alanları da dahil olmak üzere tüm ayarları
       * setting_key üzerinden kesin olarak kaydediyoruz.
       * Özellikle brand_letter_1 / brand_letter_2 / brand_name
       * için eski kayıt formatlarından kaynaklanabilecek sorunları
       * önlemek amacıyla önce mevcut design_* kayıtlarını buluyoruz.
       */
      ...
      /* Aynı setting_key daha önce birden fazla kez oluşmuşsa,
       * bulunan TÜM kayıtları aynı değere çekiyoruz. Bu, eski sürümlerde
       * oluşmuş duplicate kayıtların marka alanlarını eski değere
      ...
      (existing||[]).forEach(row=>{
        ...
        existingMap.get(row.setting_key).push(row);
      });
      ...
      for(const row of rows){
        const oldRows=existingMap.get(row.setting_key)||[];
        if(oldRows.length){
          oldRows.forEach(old=>{
            if(old.id!=null){
              updates.push({
      ...
      /*
       * Özellikle marka alanlarını doğruluyoruz. Böylece kullanıcı
       * Kaydedildi mesajını görüp sayfayı yenilediğinde eski değer
       * dönmez.
       */
      ...
      (check||[]).forEach(row=>{
        if(row.setting_key && !checkMap.has(row.setting_key)){
          checkMap.set(row.setting_key,String(row.setting_value ?? ""));
        }
      });

      const failedKey=rows.find(row=>checkMap.get(row.setting_key)!==row.setting_value);
      if(failedKey){
        throw new Error('"'+failedKey.setting_key+'" değeri veritabanında doğrulanamadı.');
      }
